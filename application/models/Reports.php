	...
	public function todays_sales_report($per_page = null, $page = null)
{
    ...
    $dateRange = "a.date = '$today'"; // only today's invoices
...
public function dashboard_totals()
{
    ...
    // 1. Total purchase
    $CI->db->select('SUM(total_amount) AS total_purchase');
    $purchase = $CI->db->get('product_purchase_details')->row_array();

    // 2. Total sales
    $CI->db->select('SUM(total_price) AS total_sales');
    $sales = $CI->db->get('invoice_details')->row_array();

    // 3. Total expense
    $CI->db->select('SUM(amount) AS total_expense');
    $expense = $CI->db->get('expense')->row_array();

    // 4. Total paid salaries
    $CI->db->select('SUM(total_salary) AS total_salary_paid');
    $CI->db->where('payment_due', 'paid');
    $salary = $CI->db->get('employee_salary_payment')->row_array();

    // 5. Total paid amount per distinct invoice_id
    $CI->db->select('SUM(paid_amount) AS total_paid');
    $CI->db->from('(SELECT invoice_id, paid_amount FROM invoice_details GROUP BY invoice_id) AS subquery');
	...
	public function retrieve_dateWise_SalesReports($start_date, $end_date)
{
    ...
    $this->db->select_sum('amount');  // Assuming 'amount' is your expense column
    $query = $this->db->get('expense'); // Your expense table name
