...
//new expense
//expense detail
public function expense_detail($type = null) {
    $data['title'] = display('expense_detail');

    // Decode the type from the URL (if encoded)
    $type = urldecode($type);
   ...
   $content     = $this->parser->parse('expense/expense_detaill', $data, true);
          $this->template->full_admin_html_view($content);  
