	...
	public function invoice_html_data($invoice_id)
{
    ...
    // Convert paid amount to float removing commas
    $paid_amount_number = floatval(str_replace(',', '', $invoice_detail[0]['paid_amount']));

    // Convert to words and add to data array
    $data['paid_amount_in_words'] = number_to_words($paid_amount_number) . ' birr only';

    // Parse and return invoice HTML
