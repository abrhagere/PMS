		...
		if(!empty($salesData)){	
			foreach($salesData as $k=>$v){
				...
				$salesData[$k]['paid_amount_formatted'] = number_format($v['paid_amount'], 2, '.', ',');
                 ...
                 $salesData[$k]['due_amount'] = number_format($due, 2, '.', ',');
				 $salesData[$k]['manufacturer_price_formatted'] = number_format($v['manufacturer_rate'], 2, '.', ',');
				 $grandProfit = $v['total_price'] - ($v['manufacturer_rate'] * $v['quantity']);
    $salesData[$k]['grand_profit'] = number_format($grandProfit, 2, '.', ',');
	 $totalGrandProfit += $grandProfit;
	  // Calculate paid profit
    $paidProfit = $v['paid_amount'] - ($v['manufacturer_rate'] * $v['quantity']);
    $salesData[$k]['paid_profit'] = number_format($paidProfit, 2, '.', ',');
	$totalPaidProfit += $paidProfit;
			}
		}
		...
		$data = array(
				...
				'totalGrandProfit'    => number_format($totalGrandProfit, 2, '.', ','),
                'totalPaidProfit'     => number_format($totalPaidProfit, 2, '.', ','),
