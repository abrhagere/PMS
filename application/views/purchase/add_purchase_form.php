...
<div class="content-wrapper">
 ...
 <div class="row">
        ...
        if($this->permission1->method('add_purchase','create')->access()){ ?>
                      <button type="button" class="btn btn-info m-b-5 m-r-2" data-toggle="modal" data-target="#purchase_csv_modal">Upload CSV Purchase</button>
                  <?php } ?>
                </div>
            </div>
        </div>
                                ...
                                <tbody id="addPurchaseItem">
                                    <tr>
                                       ...
                                       <td class="wt">
                                            ...
                                            <!-- selling price-->
                                             <td class="test1">
                                                <input type="text" name="sale_rate[]" id="sell_rate_' + count + '" class="form-control sell_rate_' + count + '" />
</td>
        ...
        select: function(event, ui) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var valueToSet = obj.total_product < 1 ? 0 : obj.total_product;
                   $('#' + available_quantity).val(valueToSet);
...
var count = 2; // start from 2 if first row exists
var limits = 500;
...
function addPurchaseOrderField1(divName) {
    if (count == limits) {
        ...
        var tabindex = count * 7;

        var tab1 = tabindex + 1; // product_name
        var tab2 = tabindex + 2; // batch_id
        var tab3 = tabindex + 3; // expire_date
        var tab4 = tabindex + 4; // quantity
        var tab5 = tabindex + 5; // product_rate
        var tab6 = tabindex + 6; // sale_rate
        var tab7 = tabindex + 7; // delete button
        var tab8 = tab7 + 1; // add_purchase button
        ...
        '<td class="test1"><input type="text" name="sale_rate[]" id="sell_rate_' + count + '" class="form-control sell_rate_' + count + ' text-right" placeholder="0.00" value="" min="0" tabindex="' + tab6 + '" required/></td>' +
        '<td class="text-right"><input class="form-control total_price text-right total_price_' + count + '" type="text" name="total_price[]" id="total_price_' + count + '" value="0.00" readonly/></td>' +
        '<td><button class="btn btn-danger red" type="button" value="<?php echo display("delete")?>" onclick="deleteRow(this)" tabindex="' + tab7 + '"><?php echo display("delete")?></button></td>';
        ...
        $("select.form-control:not(.dont-select-me)").select2({ placeholder: "Select option", allowClear: true });
    }
}
