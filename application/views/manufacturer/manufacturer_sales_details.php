 ...
 <script>
    ...
    $('#salesTable').DataTable({
        ...
        footerCallback: function(row, data, start, end, display) {
            ...
            // Helper to parse number strings (remove currency symbol, commas, etc.)
            var parseValue = function(i) {
                if (typeof i === 'string') {
                    // Remove anything not digit, dot or minus
                    i = i.replace(/[^0-9\.\-]+/g, '');
                }
                var f = parseFloat(i);
                return isNaN(f) ? 0 : f;
            ...
            // Sum over *current visible* (filtered / searched / paginated) rows:
            var pageTotal = api
                .column(4, { page: 'current' })  // change 4 to your "amount" column index
                .data()
                .reduce(function(a, b) {
                    return parseValue(a) + parseValue(b);
                }, 0);

            // If you want sum over *all rows* (ignoring filters), you can also do:
            // var totalAll = api
            //     .column(4)
            //     .data()
            //     .reduce(function(a, b) {
            ...
            // Format the result (two decimals, comma separators if needed)
            var formatted = pageTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });

            // Update footer cell
            $(api.column(4).footer()).html(formatted);
        }
    });
});
