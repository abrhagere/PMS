                            ...
                            <table class="table table-striped table-bordered" cellspacing="0" width="100%" id="InvList"> 
                                <thead>
                                    <tr>
                                        ...
                                        <th><?php echo display('manufacturer_total') ?></th> <!-- ✅ new column -->
                                         <th><?php echo display('grand_profit') ?></th>  <!-- new -->
                                         <th><?php echo display('paid_profit') ?></th>   <!-- new -->
...
//data.status = $('#status').val();
    ...
    { data: 'grand_profit', class: "grand_profit" },  // ✅ new
    { data: 'paid_profit', class: "paid_profit" },    // ✅ new
    { data: 'button' }
],
  ...
  "footerCallback": function(row, data, start, end, display) {
    ...
    // Sum Manufacturer Total
api.columns('.total_manufacturer', { page: 'current' }).every(function() {
    var sum = this
        .data()
        .reduce(function(a, b) {
            var x = parseFloat(a) || 0;
    ...
    // Sum Paid Amount
    api.columns('.paid_amount', { page: 'current' }).every(function() {
        var sum = this
            .data()
            .reduce(function(a, b) {
                var x = parseFloat(a) || 0;
    ...
    // Sum Due Amount
    api.columns('.due_amount', { page: 'current' }).every(function() {
        var sum = this
            .data()
            .reduce(function(a, b) {
                var x = parseFloat(a) || 0;
    ...
    // Sum Grand Profit
api.columns('.grand_profit', { page: 'current' }).every(function() {
    var sum = this
        .data()
        .reduce(function(a, b) {
            var x = parseFloat(a) || 0;
...
// Sum Paid Profit
api.columns('.paid_profit', { page: 'current' }).every(function() {
    var sum = this
        .data()
        .reduce(function(a, b) {
            var x = parseFloat(a) || 0;
