        ...
        <div class="panel panel-default thumbnail"> 
            ...
            <div class="panel-body">
                ...
                <table width="100%" class="datatable table table-striped table-bordered table-hover">
                    ...
                    <tfoot>
    <tr>
        <th colspan="5" style="text-align:right;"><?php echo display('total_paid_salary') ?>:</th>
        <th style="text-align:right;" colspan="5"></th> <!-- Total Salary sum goes here -->
...
$(document).ready(function() {
    var table = $('.datatable').DataTable({
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            // Total salary only for 'paid' rows (payment_type column index 6)
            var totalPaidSalary = 0;
            api.rows({page:'current'}).every(function(rowIdx, tableLoop, rowLoop) {
                var dataRow = this.data();
                var paymentType = $(dataRow[6]).text() || dataRow[6]; // Column 6 = payment_type
                if(paymentType.trim().toLowerCase() === 'paid') {
                    var salary = parseFloat((dataRow[5]+"").replace(/,/g,'')) || 0; // Column 5 = total_salary
                    totalPaidSalary += salary;
                }
            });

            // Update footer
            $(api.column(5).footer()).html(totalPaidSalary.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}));
        }
    });
